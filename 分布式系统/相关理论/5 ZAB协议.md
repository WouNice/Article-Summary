...
设计一个分布式系统必定会遇到一个问题——因为分区容忍性（partition tolerance）的存在，就必定要求我们需要在系统可用性（availability）和数据一致性（consistency）中做出权衡，这就是著名的 CAP 定理。ZooKeeper的处理方式，保证了 CP（数据一致性）。

## 什么是 ZAB 协议？

ZAB 协议全称：Zookeeper Atomic Broadcast（Zookeeper 原子广播协议）。

1、Zookeeper 是一个为分布式应用提供高效且可靠的分布式协调服务。在解决分布式一致性方面，Zookeeper 并没有使用 Paxos，而是采用了 ZAB 协议。在 ZooKeeper 的官方文档中指出，ZAB 协议并不像 Paxos 算法那样，是一种通用的分布式一致性算法，它是一种特别为 Zookeeper 设计的崩溃可恢复的原子消息广播算法。

2、ZAB 协议定义：**ZAB 协议是为分布式协调服务 Zookeeper 专门设计的一种支持 `崩溃恢复` 和 `原子广播` 协议**。
...
![alg-zab-1](./assets/alg-zab-1.png)
...
![alg-zab-2](./assets/alg-zab-2.png)

2、等待 Follwer 回应 Ack，最低超过半数即成功

![alg-zab-3](./assets/alg-zab-3.png)

3、当超过半数成功回应，则执行 commit，同时提交自己

![alg-zab-4](./assets/alg-zab-4.png)
...
![alg-zab-5](./assets/alg-zab-5.png)
...
![alg-zab-6](./assets/alg-zab-6.png)
