...
我们经常使用的分布式缓存 Redis 的官方集群解决方案（3.0 版本引入），Redis Cluster就是基于 Gossip 协议来实现集群中各个节点数据的最终一致性。
...
可以把反熵中的熵理解为节点之间数据的混乱程度/差异性，反熵就是指消除不同节点中数据的差异，提升节点间数据的相似度，从而降低熵值。
...
![Gossip 传播示意图](assets/Gossip 传播示意图.gif)

伪代码如下：

![](./assets/img-wrye-e.png)

谣言传播比较适合节点数量比较多或者节点动态变化的场景，不过，这种模式下要尽量避免传播的信息包不能太大，避免网络消耗太大。
