...
![ICMP目标不可达消息](./assets/ICMP目标不可达消息.jpg)
...
| 代码类型 | 内容                                | 种类         |
| -------- | ----------------------------------- | ------------ |
| 0        | 回送应答(EchoReply）                | 查询报文类型 |
| 3        | 目标不可达(Destination Unreachable) | 差错报文类型 |
| 4        | 原点抑制（SourceQuench）            | 差错报文类型 |
| 5        | 重定向或改变路由（Redirect）        | 差错报文类型 |
| 8        | 回送请求(EchoRequest）              | 查询报文类型 |
...
![ICMP 时间超过消息](./assets/ICMP 时间超过消息.png)
...
![主机 A ping 主机 B 期间发送的事情](./assets/主机 A ping 主机 B 期间发送的事情.png)
