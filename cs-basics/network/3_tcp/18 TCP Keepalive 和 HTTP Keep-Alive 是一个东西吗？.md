...
结论：**这两个完全是两样不同东西**，实现的层面也不同：

- TCP 的 Keepalive，是由 **TCP 层（内核态）** 实现的，称为 TCP 保活机制；主要目的是检测一个**空闲**的 TCP 连接是否仍然有效。
- HTTP 的 Keep-Alive，是由**应用层（用户态）** 实现的，称为 HTTP 长连接；目的是让一个 TCP 连接在**一次 HTTP 请求/响应**完成后**不立即关闭**，后续的 HTTP 请求可以**复用**这个已经建立的 TCP 连接，从而减少重复建立 TCP 连接（和可能的 TLS/SSL 握手）带来的延迟和开销，提高效率。
...
![请求 - 应答](./assets/请求 - 应答.png)

由于 HTTP 是基于 TCP 传输协议实现的，客户端与服务端要进行 HTTP 通信前，需要先建立 TCP 连接，然后客户端发送 HTTP 请求，服务端收到后就返回响应，至此「请求 - 应答」的模式就完成了，随后就会释放 TCP 连接。

![一个 HTTP 请求](./assets/一个 HTTP 请求.png)
...
![右边为 HTTP 流水线机制](./assets/右边为 HTTP 流水线机制.png)

但是**服务器还是按照顺序响应**，先回应 A 请求，完成后再回应 B 请求。

而且要等服务器响应完客户端第一批发送的请求后，客户端才能发出下一批的请求，也就说如果服务器响应的过程发生了阻塞，那么客户端就无法发出下一批的请求，此时就造成了「队头阻塞」的问题，HTTP的对头阻塞问题在HTTP2进行了解决。
...
| Retransmission # | RTO (ms) | Time before a timeout | Time before a timeout |
| ---------------- | -------- | --------------------- | --------------------- |
...
RFC 5482中增加了TCP_USER_TIMEOUT这个配置，通常用于定制当TCP网络连接中出现数据传输问题时，可以等待多长时间前释放网络资源。
...
TCP_USER_TIMEOUT相较SocketTimeout可以做到更精确(不影响慢查询)，SocketTimeout超时是不区分ACK还是请求响应时间的，但是TCP_USER_TIMEOUT要求下层的API、OS都支持。比如JDK不支持TCP_USER_TIMEOUT，但是Netty框架自己搞了Native来实现对TCP_USER_TIMEOUT以及其它OS参数的设置，在这些基础上Redis的Java客户端lettuce依赖了Netty，所以也可以设置TCP_USER_TIMEOUT。
...
到这里可能会有疑问：**如果tcp未开启keepalive，而应用层如http开启了长连接，会发生什么**？
