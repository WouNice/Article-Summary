...
TCP 是一个可靠传输的协议，那它是如何保证可靠的呢？为了实现可靠性传输，需要考虑很多事情，例如数据的破坏、丢包、重复以及分片顺序混乱等问题。如不能解决这些问题，也就无从谈起可靠传输。那么，TCP 是通过序列号、确认应答、重发控制、连接管理以及窗口控制等机制实现可靠性传输的。
...
![超时重传的两种情况](./assets/超时重传的两种情况.jpg)
...
![超时时间较长与较短](./assets/超时时间较长与较短.jpg)
...
![RTO 应略大于 RTT](./assets/RTO 应略大于 RTT.jpg)
...
>   $DevRTT = R1/2$
>
>   $RTO=\mu^{\star}SRTT+\partial^{\star}DevRT=\mu^{\star}R1+\partial^{\star}\left(R1/2\right)$
>
>   ----------
>
...
>   ----------
>
>   $SRTT=SRTT+\alpha\left(RTT-SRTT\right)=R1+\alpha^{\star}\left(R2-R1\right)$
>
>   $DevRTT=\left(1-\beta\right)^{*}DevRTT+\beta^{*}\left(\left|RTT-SRTT\right|\right)=\left(1-\beta\right)^{*}\left(R1/2\right)+\beta^{*}\left(\left|R2-R1\right|\right)$
>
>   $RTO=μ*SRTT+a*DevRTT$
>
>   ----------
...
快速重传机制只解决了一个问题，就是超时时间的问题，但是它依然面临着另外一个问题。就是重传时，**是重传一个，还是重传所有的问题**。
...
![用滑动窗口方式并行处理](./assets/用滑动窗口方式并行处理.jpg)
...
![可用窗口耗尽](./assets/可用窗口耗尽.jpg)

在下图，当收到之前发送的数据 `32~36` 字节的 ACK 确认应答后，如果发送窗口的大小没有变化，则**滑动窗口往右边移动 5 个字节，因为有 5 个字节的数据被应答确认**，接下来 `52~56` 字节又变成了可用窗口，那么后续也就可以发送 `52~56` 这 5 个字节的数据了。

![32 ~ 36 字节已确认](./assets/32 ~ 36 字节已确认.jpg)

> 程序是如何表示发送方的四个部分的呢？

TCP 滑动窗口方案使用三个指针来跟踪在四个传输类别中的每一个类别中的字节。其中两个指针是绝对指针（指特定的序列号），一个是相对指针（需要做偏移）。

![SND.WND, SND.UN, SND.NXT](./assets/SND.WND. SND.UN. SND.NXT.jpg)
...
TCP接收到数据包时，并不会马上交到应用层处理，或者说应用层并不会立即处理。实际上，TCP将接收到的数据包保存在接收缓存里，然后应用程序主动从缓存读取收到的分组。这样一来，如果TCP接收数据报到缓存的速度**大于**应用程序从缓存中读取数据报的速度，多个包就会被缓存，应用程序就有可能读取到多个收尾相接粘到一起的包。
...
如果一直无脑的发数据给对方，但对方处理不过来，那么就会导致触发重发机制，从而导致网络流量的无端浪费。

为了解决这种现象发生，**TCP 提供一种机制可以让「发送方」根据「接收方」的实际接收能力控制发送的数据量，这就是所谓的流量控制。**

流量控制就是根据接收方的滑动窗口的大小来控制的。比如服务端繁忙，无法及时的处理掉接收的数据，就会减小接收窗口的大小
...
![窗口关闭潜在的危险](./assets/窗口关闭潜在的危险.jpg)
...
![拥塞发送——超时重传](./assets/拥塞发送——超时重传.jpg)
...
>
> 1.拥塞避免这一段，蓝色字体：每当收到一个ACK 时，cwnd 增加 1/cwnd。是否应该是1/ssthresh?否则不符合线性增长。
>
> 2.快速重传的拥塞发生算法，步骤一和步骤 2 是否写反了？否则快速恢复算法中最后一步【如果收到新数据的 ACK 后，设置 cwnd 为ssthresh，接看就进入了拥塞避免算法】没什么意义。
>
