...
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_abort_on_overflow
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_adv_win_scale
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_allowed_congestion_control
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_app_win
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_autocorking
-r--r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_available_congestion_control
-r--r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_available_ulp
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_backlog_ack_defer
-rw-r--r--. 1 root root 0 Jul 24 23:07 /proc/sys/net/ipv4/tcp_base_mss
.....
```
...
![三次握手与数据传输](./assets/三次握手与数据传输.jpg)
...
```v
# tcp_syn_retries控制SYN包重传的次数，默认值是5次
...
```v
$ netstat -s I grep "SYNs to LISTEN"
...
![开启 syncookies 功能](./assets/开启 syncookies 功能.jpg)
...
```v
# tcp_synack_retries控制SYN+ACK包重传的次数，默认值是5次
...
![ accept 队列溢出](./assets/accept 队列溢出.jpg)
...
```v
#打开后，则当accpet队列满了会回RST，默认值是0关闭
$ echo l > /proc/sys/net/ipv4/tcp_abort_on_overflow
```
...
![常规 HTTP 请求](./assets/常规 HTTP 请求.jpg)

在 Linux 3.7 内核版本之后，提供了 TCP Fast Open 功能，这个功能可以减少 TCP 连接建立的时延。

> 接下来说说，TCP Fast Open 功能的工作方式。

![开启 TCP Fast Open 功能](./assets/开启 TCP Fast Open 功能.jpg)
...
![TCP option 字段 - TFO](./assets/TCP option 字段 - TFO.png)
...
![TIME-WAIT 时间过短，没有确保连接正常关闭](./assets/TIME-WAIT 时间过短，没有确保连接正常关闭.png)

服务端收到这个 RST 并将其解释为一个错误（Connection reset by peer），这对于一个可靠的协议来说不是一个优雅的终止方式。

为了防止这种情况出现，客户端必须等待足够长的时间，确保服务端能够收到 ACK，如果服务端没有收到 ACK，那么就会触发 TCP 重传机制，服务端会重新发送一个 FIN，这样一去一来刚好两个 MSL 的时间。

![TIME-WAIT 时间正常，确保了连接正常关闭](./assets/TIME-WAIT 时间正常，确保了连接正常关闭.png)
...
- 2MSL（TIME_WAIT 状态的持续时间）问题就不复存在了，因为重复的数据包会因为时间戳过期被自然丢弃；
- 还可以防止序列号绕回，也是因为重复的数据包会由于时间戳过期被自然丢弃；

时间戳是在 TCP 的选项字段里定义的，开启了时间戳功能，在 TCP 报文传输的时候会带上发送报文的时间戳。

![TCP option 字段 - 时间戳](./assets/TCP option 字段 - 时间戳.png)
...
| 调整 time_wait状态的上限个数                        | tcp_max_tw_buckets           |
...
![按数据包进行确认应答](./assets/按数据包进行确认应答.jpg)

所以，这样的传输方式有一个缺点：数据包的**往返时间越长，通信的效率就越低**。

**要解决这一问题不难，并行批量发送报文，再批量确认报文即可。**

![并行处理](./assets/并行处理.jpg)
...
![TCP option 选项 - 窗口扩展](./assets/TCP option 选项 - 窗口扩展.png)
...
这里需要说一个概念，就是带宽时延积，它决定网络中飞行报文的大小，它的计算方式：$带宽时延积BDP=RTT*带宽$
...
> somaxconn 和 backlog 是不是都是指的是 accept 队列？然后 somaxconn 是内核参数，backlog 是通过系统调用间隔地修改 somaxconn，比如 Linux 中 listen() 函数？

两者取最小值才是 accpet 队列。

> “如果 accept 队列满了，那么 server 扔掉 client 发过来的 ack”，也就是说该 TCP 连接还是位于半连接队列中，没有丢弃吗？”

1. 当 accept 队列满了，后续新进来的 syn 包都会被丢失
2. 文章的突发流量例子是，那个连接进来的时候 accept 队列还没满，但是在第三次握手的时候，accept 队列突然满了，就会导致 ack 被丢弃，就一直处于半连接队列。
