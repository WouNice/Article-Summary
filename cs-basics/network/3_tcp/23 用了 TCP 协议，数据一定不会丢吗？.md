...
首先，聊天软件客户端要通信，中间会通过服务器，大概长这样：

![聊天软件三端通信](./assets/聊天软件三端通信.png)

但为了**简化模型**，我们把中间的服务器给省略掉，假设这是个端到端的通信。且为了保证消息的可靠性，我们盲猜它们之间用的是**TCP 协议**进行通信。

![聊天软件两端通信](./assets/聊天软件两端通信.png)
...
![网络发包收包全景图](./assets/网络发包收包全景图.png)
...
![半连接队列和全连接队列](./assets/半连接队列和全连接队列.png)
...
![RingBuffer 满了导致丢包](./assets/RingBuffer 满了导致丢包.png)
...
![tcp_sendmsg 逻辑](./assets/tcp_sendmsg 逻辑.png)
...
![recv_buffer 丢包](./assets/recv_buffer 丢包.png)
...
![ping 查看丢包](./assets/ping 查看丢包.png)
...
![使用 TCP 协议却发生丢包](./assets/使用 TCP 协议却发生丢包.png)
