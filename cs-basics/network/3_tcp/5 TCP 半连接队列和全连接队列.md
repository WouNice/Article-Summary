...
$ ss -nt
State           Recv-Q           Send-Q      Local Address:Port     Peer Address:Port
ESTAB           0                52        192.168.134.130:22      192.168.134.1:63940
```
...
Running 1m test @ http://192.168.3.200:8088
	6 threads and 3oooo connections
    # 压测中，阻塞...
```
...
Sun May 17 07:35:40 CST 2020
	4ll50 times the listen queue of a socket overflowed
...
从上面的模拟结果可知，**当服务端并发处理大量请求时，如果 TCP 全连接队列过小，就容易溢出**。**发生 TCP 全连接队溢出的时候，后续的请求就会被丢弃，这样就会出现服务端请求数量上不去的现象**。

![](./assets/img-wtie-o.png)
...
![img-wtie-wq](./assets/img-wtie-wq.png)
...
现在我们重新压测，把 TCP 全连接队列**设置大点**，把 `somaxconn` 设置成 5000：
...
```v
$ ss -lnt | grep 8088
LISTEN            695                 5000           *:8088              *:*

$ ss -lnt | grep 8088
LISTEN            764                 5000           *:8088              *:*

$ ss -lnt | grep 8088
LISTEN            1504                5000           *:8088              *:*

$ ss -lnt | grep 8088
LISTEN            101                 5000           *:8088              *:*
```
...
![](./assets/img-wtie-et.png)

从源码中，我可以得出共有三个条件因队列长度的关系而被丢弃的：

![](./assets/img-werw-rq.jpg)
...
$ echo 128 > /proc/sys/net/core/somaxconn# 依然保持默认值128
# Nginx 的 backlog 依然保持默认值511
```
...
![](./assets/img-wtie-rw.png)

客户端执行 hping3 发起 SYN 攻击：

```c
# 客户端发起SYN 攻击
$ hping3 -S -p 8088 --flo0d 192.168.3.200
```
...
![开启 syncookies 功能](./assets/开启 syncookies 功能.png)
