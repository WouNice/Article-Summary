...
>   因为第二次挥手和第三次挥手中间是服务端的closewait状态嘛，他意思是等closewait状态过了，再一起发送fin+ack
>
>   我一下子感觉有点懵，我的回答是不建议这样做，可能会造成第一次挥手的fin报文因为收不到ack而触发超时重发，导致网络拥塞
...
- 服务端收到了 FIN 报文，然后马上回复一个 ACK 确认报文，此时服务端进入 CLOSE_WAIT 状态。在收到 FIN 报文的时候，TCP 协议栈会为 FIN 包插入一个文件结束符 EOF 到接收缓冲区中，服务器可以通过 read 调用来感知这个 FIN 包，这个 EOF 会被**放在已排队等候的其他已接收的数据之后**，所以必须要得继续 read 接收缓冲区已接收的数据；
- 接着，当服务端在 read 数据的时候，最后自然就会读到 EOF，接着 **read() 就会返回 0，这时服务器如果有数据要发送的话，就发完数据后才调用关闭连接的函数，如果服务器没有数据要发送的话，可以直接调用关闭连接的函数**，这时服务端就会发一个 FIN 包，这个  FIN 报文代表服务端不会再发送数据了，之后处于 LAST_ACK 状态；
...
服务器收到客户端的 FIN 报文时，内核会马上回一个 ACK 应答报文，但是服务器可能还有数据要发送，所以并不能马上发送 FIN 报文，而是将发送 FIN 报文的控制权交给服务器：

   - 如果服务器有数据要发送的话，就发完数据后，才调用关闭连接的函数；
   - 如果服务器没有数据要发送的话，可以直接调用关闭连接的函数，
...
当服务端收到 RST 后，内核就会释放连接，当服务器再次发起读操作或者写操作时，就能感知到连接已经被释放了：
...
![优雅关闭.drawio.png](./assets/优雅关闭.drawio.png.png)
...
当被动关闭方（上图的服务端）在 TCP 挥手过程中，「**没有数据要发送**」并且「开启了 TCP 延迟确认机制」，那么第二和第三次挥手就会**合并传输**，这样就出现了三次挥手。
...
begin listen....
accept success
received 19 bytes: hello, i am xiaolin
client closed
...
```c
    while (1) {
        ...
        if (n < 0) { // 读取错误
            ...
            // 1 表示开启 TCP_QUICKACK，即关闭 TCP 延迟确认机制
            int value = 1;
            if(setsockopt(socketfd, IPPROTO_TCP, TCP_QUICKACK, (char*)&value, sizeof(int))<0) {
                fprintf(stderr, "setsockopt TCP_QUIcKACK error:ss\n\a", strerror(errno));
                exit(1);
            }
            // --------------------------------------------------------------
            close(clientfd); // 没有数据要发送，立马关闭连接
