...
这时候，我们可选项一般也就 TCP 和 UDP 二选一。TCP 可靠，UDP 不可靠。对于普通的程序员来说，只要稍微对可靠性有些要求，普通人一般无脑选 TCP 就对了。
...
![remoteFunc](./assets/remoteFunc.png)
...
首先要向某个服务器发起请求，你得先建立连接，而建立连接的前提是，你得知道**IP 地址和端口**。这个找到服务对应的IP和端口的过程，其实就是**服务发现**。
...
![connection_pool](./assets/connection_pool.png)
